...
```{r}
  ...
  data(seattle_sales)
  ex_df <- seattle_sales %>% dplyr::filter(sale_date >= as.Date('2013-01-01') & 
                                             area == 44)
  ...
  for (k in 1:nrow(sim_df)){
    iceall_df[[k]] <- pdp::partial(object = rf_model,
                                   train = sim_df[k, ],
                                   pred.var = "trans_period",
                                   pred.grid = data.frame(trans_period = 1:max(
                                     exhed_df$trans_period))) %>% 
      dplyr::mutate(obs = k)
  }
  
  iceall_df <- iceall_df %>% dplyr::bind_rows(., pd_df %>% dplyr::mutate(obs = 0))
